...
function aiPlanEngineering(player, sunPos, efx) {
  ...
  // Infrastructure only pays off if there's farmland left to use it on — cap hardware at
  // half the grid so the AI doesn't pave over every tile and starve itself of room to
  // actually plant seeds by round 3-4.
  const hwCap=Math.floor(grid.length/2);
  const hwCards=player.hand.filter(c=>c.t==="hw");
  for(const card of hwCards){
    if(grid.filter(t=>t.c&&t.c.t==="hw").length>=hwCap) break;
    const emptyTiles=grid.filter(t=>!t.c&&t.dmg===0);
...
function makeState(names, aiFlags) {
  return {
    ...
    players:names.map((name,id)=>({id,name,isAI:(aiFlags&&aiFlags[id])||null,
...
// Ambient backdrop on the main game screen — Earth visibly degrades as Vitality drops,
// fixed to the bottom of the viewport so it's always present behind the panels.
function VitalityEarth({vit}){
  return <img src={vitEarthImg(vit)} alt="" draggable={false} style={{position:"fixed",left:"50%",bottom:"-18%",transform:"translateX(-50%)",width:"min(640px,80vw)",opacity:0.4,zIndex:0,pointerEvents:"none"}}/>;
}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(!p||!p.isAI) return;
    const t=setTimeout(()=>{
      const{ship,invest}=aiContribution(p, state.round, state.vit, state.efx, state.players.length, p.isAI);
  ...
  useEffect(()=>{
    if(connectMode==="online") return;
    if(state.phase!=="trade"||state.efx.embargo) return;
      ...
      <div style={{display:"flex",flexDirection:isNarrow?"column":"row",gap:16,padding:"16px 24px",width:"min(1700px,96vw)",margin:"0 auto"}}>
        ...
        <div style={{flex:1,minWidth:0,fontSize:13}}>
          ...
          {phase==="engineering"&&(()=>{
            ...
            return(
              <div style={S.panel}>
                <div style={S.h2}>Engineering — {p?.name}{p?.isAI?` 🤖${personalityOf(p.isAI)?.icon||""}`:""} ({engIdx+1}/{players.length})</div>
                {p?.isAI?(
                  <div style={{...S.panel,textAlign:"center",padding:24}}>
            ...
            {players.map((p,pi)=>{
              ...
              return(
                <div key={p.id} style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #0d1526",display:"flex",gap:8,alignItems:"flex-start"}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,color:"#c0ccdd",fontWeight:"bold",marginBottom:3}}>{p.name}{p.isAI?` 🤖${personalityOf(p.isAI)?.icon||""}`:""}</div>
